...
class TianaiCaptcha {
  ...
  doGenerateHtml(data) {
    ...
    if (["slider","rotate"].includes(data.type)) {
      ...
      targetBg.style.height = ((backgroundImageHeight - data.data.viewData.randomY) / backgroundImageHeight) * 185 + "px";
    ...
    setTimeout(() => {
      ...
      } else if ("concat" === data.type) {
        this.validValue.bgImageWidth = this.validValue.bgImageWidth - (this.config.width / 8).toFixed(0);
      } else if ("rotate" === data.type) {
        this.validValue.bgImageWidth = this.validValue.bgImageWidth - (targetBg.lastElementChild.offsetWidth / (this.config.width / 100 - 1)).toFixed(0);
      }
    }, 500);
  ...
  valid() {
    ...
    axios
        .post("/api/resource/merchant/clientVerifyTianaiCaptcha?" + this.config.postConfig.tokenParamName + "=" + this.config.token + "&" + this.config.postConfig.appIdParamName + "=" + this.config.appId, data)
